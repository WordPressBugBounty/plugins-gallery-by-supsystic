(function ($, ajaxurl) {
  ...
  ImageOptimize.prototype.optimizeProcess = function ($optimizeInfo, galleryInfo, photos) {
    ...
    if (photos && keys.length) {
      ...
      function processOneOptimizeImage(currIndex, logObject) {
        ...
        self.$dialogWindow.currAjaxSend = oneRequest
  ...
  ImageOptimize.prototype.prepareOptimizeLogWnd = function (optParams, imgList) {
    ...
    // if not error
    if (htmlImgRows != '') {
      $compareImgWrap.html($('.sgg-iores-cmp-template').html()).append(htmlImgRows);
    } else {
      $compareImgWrap.html();
    }
  ...
  ImageOptimize.prototype.prepareOptimizeCompareImgRow = function (urlL, urlR, sizeL, sizeR) {
    var oneRow = '<div class="supsystic-io-block-row">' + '<div class="supsystic-io-block-cell sgg-iores-img-cell">';

    if (urlL && urlL != '') {
      oneRow += '<img src="' + urlL + '" class="sgg-iores-img" alt="img_left"/>';
    }
    oneRow += '<div class="sgg-iores-img-txt">' + sizeL + ' Mb</div></div>' + '<div class="supsystic-io-block-cell sgg-iores-img-cell">' + '<img src="' + urlR + '" class="sgg-iores-img" alt="img_left"/>' + '<div class="sgg-iores-img-txt">' + sizeR + ' Mb</div></div></div>';
    return oneRow;
  };
  ...
  ImageOptimize.prototype.prepareSetupButtons = function () {
    if (!this.isPrepareSetupButtonsInited) {
      ...
      $('.sgg-service-setting .sgg-setup-button')
        .off('click')
        .on('click', function (event) {
          ...
          if ($currButton.attr('data-dialog-code')) {
            ...
            $serviceSettDialog.find('[data-img-opt-sett-code="' + $currButton.attr('data-dialog-code') + '"]').removeClass('sgg-io-tab-hidden');
  ...
  CdnTransfer.prototype.prepareSetupButtons = function () {
    if (!this.isPrepareSetupButtonsInited) {
      ...
      $('.sgg-cnd-service-info .sgg-setup-button')
        .off('click')
        .on('click', function (event) {
          event.preventDefault();
          var $currButton = $(this);
  ...
  CdnTransferDialogWindow.prototype.showStep1 = function () {
    var keys = Object.keys(this.galleryIds),
      isInitError = true,
      self = this;
    if (this.$dialogWindow && keys.length && this.serviceData && this.serviceData.current && this.serviceData.setting && this.serviceData.setting[this.serviceData.current]) {
      // show button
