...
class GridGallery_Optimization_Model_ImageOptimize extends RscSgg_Mvc_Model
{
  ...
  public function extendGalleryImageOptimizeInfo(&$galleryList, $inMb = true)
  {
    if (count($galleryList)) {
      ...
      $querySel = $this->getQueryBuilder()->select('gallery_id, can_restore, last_optimize_date, service_code, size, optimized_size')->from($this->table)->where('gallery_id', 'in', $galleryIdsStr);
      ...
      if (count($optimizeInfo) > 0) {
        foreach ($galleryList as $key => $gallObj) {
          if (isset($optimizeInfo[$gallObj->id])) {
            ...
            $gallObj->optimize_percent = GridGallery_Optimization_Model_Optimization::calcOptimizePercent($optimizeInfo[$gallObj->id]->size, $optimizeInfo[$gallObj->id]->optimized_size);
  ...
  public function insertUpdate($assocArray)
  {
    ...
    $querySel = $this->getQueryBuilder()->select('gallery_id')->from($this->table)->where('gallery_id', '=', (int) $assocArray['gallery_id']);
    ...
    if (count($optimizeRecords) > 0) {
      ...
      $queryUpdate = $this->getQueryBuilder()->update($this->table)->fields(array_keys($assocArray))->values(array_values($assocArray))->where('gallery_id', '=', (int) $gallery_id);

      return $this->db->query($queryUpdate->build());
    } else {
      // insert
      $queryInsert = $this->getQueryBuilder()->insertInto($this->table)->fields(array_keys($assocArray))->values(array_values($assocArray));
  ...
  {
    $query = $this->getQueryBuilder()->deleteFrom($this->table)->where('gallery_id', '=', (int) $galleryId);
  ...
  public function getInfoByGalleryId($galleryId)
  {
    $galleryId = (int) $galleryId;
    if ($galleryId) {
      $selectQuery = $this->getQueryBuilder()->select('service_code')->from($this->table)->where('gallery_id', '=', $galleryId);
