...
class GridGallery_Optimization_Model_Cdn extends RscSgg_Mvc_Model
{
  ...
  public function extendGalleryList(&$galleryList)
  {
    if (count($galleryList)) {
      ...
      $querySel = $this->getQueryBuilder()->select('gallery_id, last_transfer_date, service_code, size')->from($this->table)->where('gallery_id', 'in', $galleryIdsStr);
  ...
  public function save($params)
  {
    if (isset($params['gallery_id']) && isset($params['last_transfer_date']) && isset($params['service_code']) && isset($params['size'])) {
      ...
      $querySel = $this->getQueryBuilder()->select('gallery_id')->from($this->table)->where('gallery_id', '=', (int) $params['gallery_id']);
      ...
      if (count($cdnRecords) > 0) {
        ...
        $queryUpdate = $this->getQueryBuilder()->update($this->table)->fields(array_keys($params))->values(array_values($params))->where('gallery_id', '=', (int) $gallery_id);

        $res = $this->db->query($queryUpdate->build());
        return true;
      } else {
        // insert
        $queryInsert = $this->getQueryBuilder()->insertInto($this->table)->fields(array_keys($params))->values(array_values($params));
  ...
  {
    $querySel = $this->getQueryBuilder()->select('service_code')->from($this->table)->where('gallery_id', '=', (int) $galleryId);
