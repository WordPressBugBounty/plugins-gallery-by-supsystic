...
class GridGallery_Insta_Controller extends GridGallery_Core_BaseController
{
  ...
  {
    return ['completeAction', 'refreshAction', 'logoutAction'];
  }
  /**
  ...
  public function indexAction(RscSgg_Http_Request $request)
  {
    ...
    if (!get_option('insta_token')) {
      return $this->redirect($this->generateUrl('insta', 'authorization', ['id' => $galleryId]));
    }

    return $this->response('@insta/index.twig', [
      'images' => get_option('insta_thumbnails'),
      'id' => $galleryId,
      'galleryName' => $galleries->getById($galleryId)->title,
    ]);
  }
  ...
  public function authorizationAction(RscSgg_Http_Request $request)
  {
    ...
    try {
      return $this->response('@insta/authorization.twig', ['url' => $client->getAuthorizationUrl(), 'id' => $galleryId, 'galleryName' => $galleries->getById($galleryId)->title]);
    } catch (Exception $e) {
      return $this->response('error.twig', ['message' => $e->getMessage()]);
    }
  }
  ...
  public function completeAction(RscSgg_Http_Request $request)
  {
    ...
    return $this->redirect($this->generateUrl('insta', 'index', ['id' => $galleryId]));
  }
  ...
  public function refreshAction(RscSgg_Http_Request $request)
  {
    ...
    return $this->response(RscSgg_Http_Response::AJAX, ['images' => $images]);
  }
