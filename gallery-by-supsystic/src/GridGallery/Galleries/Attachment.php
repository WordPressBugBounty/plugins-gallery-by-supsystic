...
class GridGallery_Galleries_Attachment
{
  ...
  public function __construct()
  {
    ...
    if (isset($this->settings['image_editor']) && $this->settings['image_editor'] !== 'auto') {
      if (!has_filter('wp_image_editors', [$this, 'forceImageEditor'])) {
        @set_time_limit(120);
        add_filter('wp_image_editors', [$this, 'forceImageEditor']);
      }
    }
  }
  ...
  {
    $cacheParams = [$attachmentId, $width, $height, $cropPosition, $cropQuality];
    ...
    if ($cropPosition && $width && $height) {
      ...
      if ((!empty($cropPositionUpdate) && ($cropPositionUpdate[0] == true || $cropPositionUpdate[1] !== $width || $cropPositionUpdate[2] !== $height)) || $cropQuality) {
        if ($url = $this->crop($attachment, $width, $height, $cropPosition, $cropQuality)) {
          update_post_meta($attachmentId, 'cropPositionNeedUpdate', [false, $width, $height], $cropPositionUpdate);
  ...
  {
    $cacheParams = [$attachment['id'], $width, $height, $cropPosition, $cropQuality];
    $res = $this->_getCache('_sgg_final_crop', $cacheParams);
    if ($res != self::$_NO_CACHE) {
      return $res;
    }
    $filepath = $this->getFilePath($attachment);
    [$or_width, $or_height] = getimagesize($filepath);
  ...
  protected function getCroppedSizeUrl($attachment, $width, $height)
  {
    ...
    $cacheParams = [$attachment['id'], $width, $height];
  ...
  public static function prepareWatermarkedImageParams($ggAreaSettings)
  {
    ...
    if (isset($ggAreaSettings['grid'])) {
      if ($ggAreaSettings['grid'] == 0 || $ggAreaSettings['grid'] == '3') {
        $ggSetting['crop'] = 1;
      } elseif ($ggAreaSettings['grid'] == '1') {
        $ggSetting['photo_height'] = null;
      } elseif ($ggAreaSettings['grid'] == '2') {
        $ggSetting['photo_width'] = null;
      }
    }
