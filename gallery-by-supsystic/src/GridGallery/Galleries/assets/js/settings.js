...
(function (app, $) {
  ...
  Controller.prototype.showReviewNotice = function () {
    ...
    $.post(window.wp.ajax.settings.url, {
      ...
      if (response.show) {
        ...
        $.merge($('#reviewNotice [data-statistic-code]').closest('button'), $('#reviewNotice').prev().find('button')).on('click', function () {
          $.post(window.wp.ajax.settings.url, {
            action: 'grid-gallery',
            _wpnonce: SupsysticGallery.nonce,
            route: {
  ...
  Controller.prototype.toggleArea = function () {
    ...
    $toggle
      .on('change', function () {
        ...
        $pagesRow
          .find('input[name="pagination[enabled]"]')
          .prop('disabled', selectedTypeVal == 1)
          .iCheck('update');
        mosaicLayoutToggle();
  ...
  Controller.prototype.toggleShadow = function () {
    ...
    $shadowTable.find('thead input[type="radio"]:checked').trigger('click').trigger('change');
  };
  ...
  Controller.prototype.togglePopUp = function () {
    ...
    $popupTable.find('thead input[type="radio"]:checked').trigger('click').trigger('change');
  };
  ...
  Controller.prototype.toggleLazyload = function () {
    ...
    $lazyloadTable.find('thead input[type="radio"]:checked').trigger('click').trigger('change');
  };
  ...
  Controller.prototype.toggleAutoPosts = function () {
    var $autoposts = $('#autoposts'),
      ...
      $autopostsElements = [$('#autopostsNumber').closest('tr'), $('#autopostsCategories').closest('tr')],
      $postsElements = [$('#posts').closest('tr'), $('#pages').closest('tr'), $('#table-toolbar'), $('#gview_ui-jqgrid-htable')];
  ...
  Controller.prototype.togglePreload = function () {
    ...
    $preloadTable.find('thead input[type="radio"]:checked').trigger('click').trigger('change');
  };
  ...
  Controller.prototype.checkForOptimization = function () {
    ...
    if (ioServiceParams != null && ioServiceParams.setting && ioServiceParams.current && ioServiceParams.setting[ioServiceParams.current] && ioServiceParams.setting[ioServiceParams.current].auth_key) {
          ...
          if (response && response.success == true && response.photos && response.galleryInfo) {
            ...
            if (photos && keys.length) {
              ...
              function processOneOptimizeImage(currIndex, logObject) {
                ...
                currAjaxSend = oneRequest
                  .send(function (opResponse) {
  ...
  Controller.prototype.checkForCdn = function () {
    ...
    if (cdnParams && cdnParams.current && cdnParams.setting && cdnParams.setting[cdnParams.current] && cdnParams.setting[cdnParams.current].zone_name && cdnParams.setting[cdnParams.current].u_name && cdnParams.setting[cdnParams.current].base_ftp_path && cdnParams.setting[cdnParams.current].u_pass) {
      var request = app.Ajax.Post({
        module: 'optimization',
...
(function ($, Gallery) {
  ...
  ImagePreview.prototype.initShadow = function () {
    ...
    $(selectors.join(','))
      ...
      .on('change paste keyup', function () {
        ...
        _this.$window.find('figure').attr('data-box-shadow', boxShadow).trigger('shadowChange');
      });
  ...
  ImagePreview.prototype.previewCaptionHide = function () {
    $('.gallery-preview .grid-gallery-caption').data('grid-gallery-type', 'none').attr('data-grid-gallery-type', 'none');
  ...
  ImagePreview.prototype.previewCaptionShow = function (fields) {
    ...
    $('#effectsPreview')
      .find('figure:not(.available-in-pro)')
      .bind(
        'click',
        $.proxy(function (e) {
    ...
    $(getSelector(fields.bg))
      .bind(
        ...
        $.proxy(function (e) {
          ...
          if (color) {
            backgroundColor = 'rgba(' + color.r + ',' + color.g + ',' + color.b + ',' + (1 - $(getSelector(fields.opacity)).val() / 10) + ')';
          }
    ...
    $(getSelector(fields.opacity)).bind(
      'change',
      $.proxy(function (e) {
  ...
  ImagePreview.prototype.initCaption = function () {
    ...
    $('[name="thumbnail[overlay][enabled]"]')
      .off('ifChanged')
  ...
  ImagePreview.prototype.setIconParams = function ($iconWrapper, params) {
    ...
    if (params) {
      ...
      if (params.fontSize) {
        ...
        if (!isNaN(fontSize)) {
          $('#sggSettingHiIconBeforeFontSizeStyle').html('.hi-icon:before {font-size: ' + fontSize + 'px !important;' + 'line-height: ' + 2 * fontSize + 'px !important;}');
          $('#sggSettingsIconSizeStyle').html('.hi-icon {width: ' + 2 * fontSize + 'px !important;' + 'height: ' + 2 * fontSize + 'px !important;}');
        }
      }
    }
  };
  ...
  ImagePreview.prototype.initCaptionEffects = function () {
    var self = this,
      figure = $('#preview figure.grid-gallery-caption');
    ...
    figure.each(function () {
      ...
      if ($(this).data('grid-gallery-type') == 'polaroid' && $(this).closest('#preview').length > 0) {
        ...
        $(this)
          .find('.gg-caption-row')
          .css({ 'max-height': figcaptionHeight ? figcaptionHeight : '100%', height: 'auto' });
