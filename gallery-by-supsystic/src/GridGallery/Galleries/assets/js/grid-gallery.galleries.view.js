...
(function (app, $) {
    ...
    this.$checkboxes.on(
      ...
      $.proxy(function (event) {
        ...
        $.each(this.$checkboxes, function (index, checkbox) {
          if (checkbox.checked) {
            checked = true;
            checkboxes.push(app.Common.getParentEntity($(checkbox)));
          }
        });
  ...
  Controller.prototype.filterImages = function () {
    ...
    $imagesTr.each(function () {
      ...
      if ($captionField.val().indexOf(filterValue) != -1 || $(this).data('entity-info').attachment.title.indexOf(filterValue) != -1) {
        $(this).show();
      } else {
        $(this).hide();
      }
    ...
    if (this.checked.length >= 0 && this.checked.length != this.$checkboxes.length) {
      this.$checkboxes.each(
        $.proxy(function (index, element) {
          ...
          if (!$element.is(':checked')) {
            $element.trigger('click').attr('checked', 'checked').iCheck('update');
          }
        }, this)
      );
      ...
      this.$checkboxes.each(
        $.proxy(function (index, element) {
          ...
          if ($element.is(':checked')) {
            $element.trigger('click').removeAttr('checked').iCheck('update');
          }
        }, this)
      );
    }
    ...
    if (show) {
      ...
      $catContainers.each(function () {
        ...
        if (category == '') {
          ...
          $entityContainer
            .find('li[data-entity-tag *=";' + category + ';"]')
  ...
  $(document).ready(function () {
    ...
    if (queryString.module === 'galleries' && (queryString.action === 'view' || queryString.action === 'addImages' || queryString.action === 'sort')) {
      ...
      if (queryString.action === 'view') {
        ...
        $('[data-button="filterimages"]').on('click', $.proxy(controller.filterImages, controller));
        ...
        if ($galleriesList.length) {
          ...
          $('.image-copy-btn, .image-move-btn').on('click', function () {
            var $elem = $(this);
            controller.copyImageTo($elem.parent().find('select.copy-option').val(), $elem.hasClass('image-move-btn'), getImageId($elem));
            return false;
          });
        } else {
      ...
      $('input#checkAll').on('click', $.proxy(controller.toggleCheckbox, controller));
