...
class GridGallery_Galleries_Model_Settings extends GridGallery_Core_BaseModel
{
  ...
  public function getCatsFromPreset($data, $config)
  {
    ...
    if (isset($customPresets[$index1]) && !$presets[$preset_number] && !empty($customPresets) && $customPresets[$index1]['categories']) {
      $data['categories'] = array_merge($data['categories'], $customPresets[$index1]['categories']);
    } else {
  ...
  public function getPagesFromPreset($data, $config)
  {
    ...
    $index1 = (int) $preset_number - sizeof($presets);
    if (isset($customPresets[$index1]) && !$presets[$preset_number] && !empty($customPresets) && $customPresets[$index1]['pagination']) {
      $data['pagination'] = array_merge($data['pagination'], $customPresets[$index1]['pagination']);
    } else {
      if (isset($presets[$preset_number]) && is_array($presets[$preset_number]) && !empty($presets[$preset_number])) {
  ...
  protected function insert($id, $data)
  {
    ...
    $query = $this->getQueryBuilder()->insertInto($this->table)->fields(array_keys($fields))->values(array_values($fields));
  ...
  {
    $query = $this->getQueryBuilder()->update($this->table)->where('gallery_id', '=', (int) $id)->fields('data')->values('%s');

    if (false !== $this->db->query($this->db->prepare($query->build(), serialize($data)))) {
      return true;
    }
  ...
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->table)->where($field, '=', $value);
  ...
  public function getSettListByRegExp()
  {
    ...
    $queryStr = 'SELECT id, gallery_id, data FROM ' . $this->table;
