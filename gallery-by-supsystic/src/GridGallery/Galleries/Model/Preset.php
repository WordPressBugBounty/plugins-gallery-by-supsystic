...
class GridGallery_Galleries_Model_Preset extends GridGallery_Core_BaseModel
{
  ...
  public function set($settingsId, $title)
  {
    ...
    $query = $this->getQueryBuilder()->insertInto($this->table)->fields('settings_id', 'title')->values((int) $settingsId, $title);
  ...
  {
    $query = $this->getQueryBuilder()->deleteFrom($this->table)->where('id', '=', (int) $presetId);
  ...
  public function getAll()
  {
    ...
    if (false === ($presets = $this->db->get_results($query->build()))) {
      throw new RuntimeException(sprintf('Failed to get presets: %s', $this->getLastError()));
    }

    if (is_array($presets)) {
      $presets = array_map([$this, 'extend'], $presets);
    }
  ...
  {
    if (!is_object($preset)) {
      throw new UnexpectedValueException(sprintf('$preset must be the object, %s given.', gettype($preset)));
    }

    $preset->settings = $this->getSettingsModel()->getById((int) $preset->settings_id);
