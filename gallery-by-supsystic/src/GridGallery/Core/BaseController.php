...
class GridGallery_Core_BaseController extends RscSgg_Mvc_Controller
{
   ...
   * @return array
   */
  public function getResponseData($isError, $message = '', $additional = [])
  {
    $data = [
      'error' => (bool) $isError,
      'message' => $message,
   ...
   * @return array
   */
  public function getErrorResponseData($message = '', $additional = [])
  {
   ...
   * @return array
   */
  public function getSuccessResponseData($message = '', $additional = [])
  {
  ...
  public function getModel($modelName)
  {
    ...
    if ($model) {
      if (method_exists($model, 'setEnvironment')) {
        $model->setEnvironment($this->getEnvironment());
      }

      if (method_exists($model, 'onInit')) {
        $model->onInit();
      }
    }
  ...
  {
    if (is_array($text)) {
      return array_map([$this, 'escape'], $text);
    }
