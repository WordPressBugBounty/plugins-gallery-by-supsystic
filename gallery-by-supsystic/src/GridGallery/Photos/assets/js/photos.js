...
(function (app, $, ajaxurl) {
  ...
  Controller.prototype.initLinkedImagesDialog = function () {
    $linkedImages = $('#linkedImagesDialog').dialog({
      ...
      buttons: {
        Save: function () {
          ...
          $('.linked-attachments-list .attachments-' + id)
            .find('img')
    ...
    $imgTemplate = $('<div class="sc-attachment">' + '<div class="thumbnail"></div>' + '</div>').on('click', function (event) {
      event.preventDefault();
    ...
    $addImages.on('click', function (event) {
      ...
      frame.on('select', function () {
        ...
        for (var i = 0; i < attachments.length; i++) {
          url = (attachments[i].sizes.thumbnail && attachments[i].sizes.thumbnail.url) || attachments[i].url;
    ...
    $removeImages.on('click', function () {
      var id = $(this).closest('#linkedImagesDialog').data('id');
      $('.linked-attachments-list .attachments-' + id + ' .sc-attachment.selected').fadeOut(300, function () {
        $(this).remove();
      });
    });
  ...
  Controller.prototype._initDialogs = function () {
    ...
    function addToGallery() {
      ...
      post.send(
        $.proxy(function (response, request) {
          if (response.message) {
    ...
    $('.gg-sort-entities').on('click', 'li', function (e) {
      if (e.altKey) {
        ...
        if (first >= 0) {
          ...
          ((from = first < current ? first : current + 1), (to = first < current ? current - 1 : first), ($all = $(this).parent().find('li')));
      ...
      $('[data-droppable]').droppable({
        ...
        drop: function (event, ui) {
          ...
          if ($('[data-observable]:checked').length > 0) {
            $.each($('[data-observable]:checked'), function (index, checkbox) {
              move(app.Common.getParentEntity(checkbox), $folder);
            });
          } else {
            move($entity, $folder);
          }
