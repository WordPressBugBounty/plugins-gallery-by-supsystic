...
class GridGallery_Photos_Model_Folders extends RscSgg_Mvc_Model
{
  ...
  public function add($title)
  {
    ...
    $query = $this->getQueryBuilder()->insertInto($this->table)->fields('title', 'date')->values($title, date('Y-m-d H:i:s'));
  ...
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->table)->where('id', '=', (int) $id);
  ...
  {
    $query = $this->getQueryBuilder()->deleteFrom($this->table)->where('id', '=', (int) $id);
