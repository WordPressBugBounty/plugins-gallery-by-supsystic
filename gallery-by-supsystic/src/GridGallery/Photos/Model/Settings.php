...
class GridGallery_Photos_Model_Settings extends RscSgg_Mvc_Model
{
  ...
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->table)->where('name', '=', $name);
  ...
  {
    $query = $this->getQueryBuilder()->update($this->table)->fields('value')->values($value)->where('name', '=', $name);
