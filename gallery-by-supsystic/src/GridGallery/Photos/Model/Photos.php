...
class GridGallery_Photos_Model_Photos extends RscSgg_Mvc_Model
{
  ...
  {
    $query = $this->getQueryBuilder()->insertInto($this->table)->fields('attachment_id', 'folder_id')->values((int) $attachmentId, (int) $folderId);
  ...
  {
    $query = $this->getQueryBuilder()->update($this->table)->fields('attachment_id')->values((int) $attachmentId)->where('id', '=', $photoId);
  ...
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->table)->where('attachment_id', '=', (int) $attachmentId)->orderBy('id')->order('DESC');
  ...
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->table)->where('folder_id', '=', (int) $folderId);
  ...
  {
    $query = $this->getQueryBuilder()->select('*')->from($this->table)->where('folder_id', '=', 0);
  ...
  {
    $caption = htmlspecialchars($caption, ENT_QUOTES, get_bloginfo('charset'));
  ...
  {
    $description = htmlspecialchars($description, ENT_QUOTES, get_bloginfo('charset'));
  ...
  {
    update_post_meta((int) $attachmentId, $this->getMetadataField('link'), $link);
  }

  public function setCaptionEffect($attachmentId, $captionEffect)
  {
    update_post_meta((int) $attachmentId, 'captionEffect', $captionEffect);
  }
  ...
  {
    update_post_meta((int) $attachmentId, $this->getMetadataField('target'), $target);
  }
  ...
  {
    update_post_meta((int) $attachmentId, $this->getMetadataField('rel'), $rel);
  }
  ...
  {
    update_post_meta((int) $attachmentId, 'cropPosition', $position);
    update_post_meta((int) $attachmentId, 'cropPositionNeedUpdate', [true, null, null]);
  }
  ...
  public function updateMetadata($attachmentId, array $metadata)
  {
    foreach ($metadata as $key => $value) {
      if (!method_exists($this, $method = sprintf('set%s', ucfirst($key)))) {
        throw new BadMethodCallException(sprintf('The method %s does not exists.', $method));
      }

      call_user_func_array([$this, $method], [(int) $attachmentId, $value]);
    }
  }
  ...
  protected function getBy($field, $identifier)
  {
    ...
    $query = $this->getQueryBuilder()->select('*')->from($this->table)->where($field, '=', $identifier);
  ...
  public function getPhotos($resourcesData)
  {
    ...
    if (count($resourcesData)) {
      ...
      $query = $this->getQueryBuilder()->select('*')->from($this->table)->where('id', 'IN', implode(',', $resourceIds));
  ...
  {
    $query = $this->getQueryBuilder()->select('COUNT(*) AS total')->from($this->table);
