...
class GridGallery_Photos_Controller extends GridGallery_Core_BaseController
{
  ...
  public function indexAction(RscSgg_Http_Request $request)
  {
    ...
    $images = array_map([$position, 'setPosition'], $photos->getAllWithoutFolders());

    return $this->response('@photos/index.twig', [
      'entities' => [
  ...
  public function viewAction(RscSgg_Http_Request $request)
  {
    if (!$request->query->has('folder_id')) {
      $this->redirect($this->getEnvironment()->generateUrl('photos', 'index'));
    }
    ...
    foreach ($folder->photos as $index => $row) {
      $folder->photos[$index] = $position->setPosition($row, 'folder', $folderId);
    }

    $folder->photos = $position->sort($folder->photos);

    return $this->response('@photos/view.twig', [
      'folder' => $folder,
      'ajax_url' => esc_url(admin_url('admin-ajax.php')),
      'view_type' => sanitize_text_field($request->query->get('view', self::STD_VIEW)),
    ]);
  ...
  {
    return $_photos->add($_attachment->ID, $_request->post->get('folder_id', 0), []);
  }
  ...
  public function addAction(RscSgg_Http_Request $request)
  {
    ...
    if ($env->getConfig()->isEnvironment(RscSgg_Environment::ENV_DEVELOPMENT)) {
    ...
    if (!$this->addPhotoResForAddAction($photos, $attachment, $request)) {
      $response = [
        'error' => true,
        'photo' => null,
        'message' => sprintf($env->translate('Unable to save chosen photo %s: %s'), esc_html($attachment->post_title), esc_html($photos->getLastError())),
      ];
    } else {
      $response = [
        'error' => false,
        'message' => sprintf($env->translate('Photo %s was successfully imported to the Grid Gallery'), esc_html($attachment->post_title)),
        'link' => $this->generateUrl('galleries', 'view', ['gallery_id' => sanitize_key($request->post->get('galleryId'))]),
      ];
    }
  ...
  public function addFolderAction(RscSgg_Http_Request $request)
  {
    ...
    if (!$folders->add($folderName ? $folderName : $env->translate('New Folder'))) {
      ...
      $folder = $env->getTwig()->render(sprintf('@ui/%s/folder.twig', $viewType ? $viewType : 'block'), ['folder' => $folders->getById($folders->getInsertId())]);
  ...
  public function deleteAction(RscSgg_Http_Request $request)
  {
    ...
    $debug = $env->getConfig()->isEnvironment(RscSgg_Environment::ENV_DEVELOPMENT);
  ...
  public function rotatePhotoAction(RscSgg_Http_Request $request)
  {
    ...
    return $this->response(RscSgg_Http_Response::AJAX, ['message' => sprintf($env->translate('There are %d photos successfully rotated'), $rotated)]);
  }
  ...
  public function renderAction(RscSgg_Http_Request $request)
  {
    ...
    foreach ($photos as $photo) {
      $renders[] = $this->getEnvironment()
        ->getTwig()
        ->render('@photos/includes/photo.twig', ['photo' => $photo]);
    }
  ...
  public function updateAttachmentAction(RscSgg_Http_Request $request)
  {
    ...
    $update = [];
    if (!is_null($request->post->get('alt'))) {
      $update['alt'] = empty($alt) ? ' ' : $alt;
    }
    if (!is_null($request->post->get('caption'))) {
      $update['caption'] = $caption;
    }
    if (!is_null($request->post->get('description'))) {
      $update['description'] = $description;
    }
    if (!is_null($request->post->get('captionEffect'))) {
      $update['captionEffect'] = $captionEffect;
    }
    if (!is_null($request->post->get('cropPosition'))) {
      $update['cropPosition'] = $cropPosition;
    }
  ...
  {
    $response = $this->getErrorResponseData($this->translate('Failed to update position.'));
    $data = (array) $request->post->get('data');

    if ($this->getModel('position')->replacePosition($data)) {
      $response = $this->getSuccessResponseData($this->translate('Position updated successfully!'));
    }
