...
class RscSgg_Menu_Page
{
  ...
  public function register()
  {
    ...
    if (is_array($this->submenu) && count($this->submenu) > 0) {
      usort($this->submenu, [$this, 'sortSubMenuItemsClb']);
      /** @var RscSgg_Menu_Item $submenu */
      foreach ($this->submenu as $submenu) {
        add_action('admin_menu', [$submenu, 'register']);
      }
    }
  }
  ...
  public function addMenuPage()
  {
    ...
    $parameters = [$this->pageTitle, $this->menuTitle, $this->getCapability(), $this->menuSlug, [$this->resolver, 'resolve'], $this->iconUrl, $this->position];
