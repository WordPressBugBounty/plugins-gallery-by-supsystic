...
class RscSgg_Environment
{
  ...
  public function __construct($pluginName, $version, $path)
  {
    ...
    add_action('init', [$this, 'wpInitCallback']);
  }
  ...
  public function run()
  {
    ...
    try {
      ...
      $this->twig = new Twig_SupTwgSgg_Environment(new Twig_SupTwgSgg_Loader_Filesystem($templatesPath), [
    ...
    if ($this->config->has('plugin_menu')) {
      ...
      foreach ($parameters = $this->config->get('plugin_menu') as $key => $value) {
        if (method_exists($this->menu, $method = sprintf('set%s', str_replace('_', '', $key)))) {
          call_user_func_array([$this->menu, $method], [$value]);
        }
      }
    ...
    add_action('plugins_loaded', [$this, 'extend']);
  }
  ...
  public function registerActivation()
  {
    ...
    register_activation_hook($index, [$this->resolver, 'install']);
  }
  ...
  public function getLang()
  {
    if ($this->lang === null) {
      ...
      if (!($path = $this->config->get('lang_path'))) {
        $path = plugin_basename(dirname(dirname(dirname(__FILE__))) . '/app/langs');
   ...
   * @return string
   */
  public function generateUrl($module, $action = 'index', array $parameters = [], $hash = '')
  {
