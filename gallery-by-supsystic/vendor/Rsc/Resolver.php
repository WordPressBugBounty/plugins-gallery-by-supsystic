...
class RscSgg_Resolver
{
  ...
  public function init()
  {
    ...
    foreach ($modules as $name => $instance) {
      ...
      register_deactivation_hook($index, [$instance, 'onDeactivation']);

      if ($name === 'installer') {
        register_uninstall_hook($index, [get_class($instance), 'onUninstall']);
      }
  ...
  public function getModulesList()
  {
    if (!$this->modulesList) {
      $modulesList = [];
      $modulesLocation = [[dirname(dirname(__FILE__)), 'Rsc'], [$this->environment->getConfig()->get('plugin_source'), $this->environment->getConfig()->get('plugin_prefix')]];

      foreach ($modulesLocation as $locationData) {
        if (!in_array(null, $locationData) && count($locationData) == 2) {
          [$path, $prefix] = $locationData;
