...
class RscSgg_Html
{
  ...
  public static function textarea($name, $params = ['attrs' => '', 'value' => '', 'rows' => 3, 'cols' => 50])
  {
    ...
    if (isset($params['required']) && $params['required'] && (!isset($params['customValidation']) || !$params['customValidation'])) {
  ...
  public static function ajaxfile($name, $params = ['url' => '', 'value' => '', 'fid' => '', 'buttonName' => ''])
  {
    ...
    $out .=
      self::inputButton(['value' => __(empty($params['buttonName']) ? 'Upload' : $params['buttonName']), 'attrs' => 'id="' . $butId . '" class="ui button primary mini"']) .
      '<div id="file-upload-wrapper" name="' .
      $name .
      '">' .
      self::input($name, ['type' => 'hidden', 'attrs' => 'id="file-upload-' . $name . '"']) .
      '</div>' .
      self::file('file-upload-' . $name, $params);
    $display = empty($params['value']) ? 'display: none;' : '';

    $out .= '<div>' . self::img($params['value'], 0, ['attrs' => 'id="prev_' . $butId . '" style="height: 200px;' . $display . '" class="previewpicture"']) . '</div>';
    ...
    $out .=
      ...
      $butId .
      '").click(function (e) {
                    jQuery(\'input[id="file-upload-input-' .
      $name .
      '"]\').click();
                });
                jQuery(\'input[id="file-upload-input-' .
      ...
      '"]\').change(function () {
                    if (jQuery(this).val()) {
                        ...
                        Membership.api.base.uploadFile(
                            {image: jQuery(\'input[id="file-upload-input-' .
      $name .
      '"]\')[0].files[0]},
                            function (attachment) {
                                jQuery("#prev_' .
      $butId .
      '").attr("src", attachment.src).css("display", "block");
                                jQuery(".registration-submit").removeAttr("disabled");
                                jQuery(\'input[id="file-upload-' .
      $name .
  ...
  public static function checkboxHiddenVal($name, $params = ['attrs' => '', 'value' => '', 'checked' => ''])
  {
    ...
    $out .=
      ...
      '").change(function(){
					jQuery("#' .
      $hideId .
      '").val( (jQuery(this).attr("checked") ? 1 : 0) );
				});
			});
			//--></script>';
    return $out;
  ...
  public static function wpEditor($name, $params = [])
  {
