...
class Twig_SupTwgSgg_NodeVisitor_Optimizer extends Twig_SupTwgSgg_BaseNodeVisitor
{
  ...
  protected function optimizePrintNode(Twig_SupTwgSgg_NodeInterface $node, Twig_SupTwgSgg_Environment $env)
  {
    ...
    if ($exprNode instanceof Twig_SupTwgSgg_Node_Expression_BlockReference || $exprNode instanceof Twig_SupTwgSgg_Node_Expression_Parent) {
  ...
  protected function enterOptimizeFor(Twig_SupTwgSgg_NodeInterface $node, Twig_SupTwgSgg_Environment $env)
  {
    ...
    elseif ($node instanceof Twig_SupTwgSgg_Node_Expression_Function && 'include' === $node->getAttribute('name') && (!$node->getNode('arguments')->hasNode('with_context') || false !== $node->getNode('arguments')->getNode('with_context')->getAttribute('value'))) {
      $this->addLoopToAll();
    }

    // the loop variable is referenced via an attribute
    elseif (
      $node instanceof Twig_SupTwgSgg_Node_Expression_GetAttr &&
      (!$node->getNode('attribute') instanceof Twig_SupTwgSgg_Node_Expression_Constant || 'parent' === $node->getNode('attribute')->getAttribute('value')) &&
      (true === $this->loops[0]->getAttribute('with_loop') || ($node->getNode('node') instanceof Twig_SupTwgSgg_Node_Expression_Name && 'loop' === $node->getNode('node')->getAttribute('name')))
    ) {
      $this->addLoopToAll();
    }
  }
