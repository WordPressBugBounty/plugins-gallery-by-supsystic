...
abstract class Twig_SupTwgSgg_Node_Expression_Call extends Twig_SupTwgSgg_Node_Expression
{
  ...
  protected function getArguments($callable, $arguments)
  {
    ...
    if ($isVariadic) {
      $arbitraryArguments = new Twig_SupTwgSgg_Node_Expression_Array([], -1);
    ...
    if (!empty($parameters)) {
      ...
      throw new Twig_SupTwgSgg_Error_Syntax(sprintf('Unknown argument%s "%s" for %s "%s(%s)".', count($parameters) > 1 ? 's' : '', implode('", "', array_keys($parameters)), $callType, $callName, implode(', ', $names)), $unknownParameter ? $unknownParameter->getTemplateLine() : -1);
    }
