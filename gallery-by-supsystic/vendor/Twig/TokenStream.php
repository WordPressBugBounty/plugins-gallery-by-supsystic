  ...
  public function expect($type, $value = null, $message = null)
  {
    ...
    if (!$token->test($type, $value)) {
      ...
      throw new Twig_SupTwgSgg_Error_Syntax(
        sprintf('%sUnexpected token "%s" of value "%s" ("%s" expected%s).', $message ? $message . '. ' : '', Twig_SupTwgSgg_Token::typeToEnglish($token->getType()), $token->getValue(), Twig_SupTwgSgg_Token::typeToEnglish($type), $value ? sprintf(' with value "%s"', $value) : ''),
        $line,
        $this->source,
      );
    }
