...
class Twig_SupTwgSgg_ExpressionParser
{
  ...
  public function parseArrayExpression()
  {
    ...
    $node = new Twig_SupTwgSgg_Node_Expression_Array([], $stream->getCurrent()->getLine());
    $first = true;
  ...
  public function parseHashExpression()
  {
    ...
    while (!$stream->test(Twig_SupTwgSgg_Token::PUNCTUATION_TYPE, '}')) {
      ...
      if (($token = $stream->nextIf(Twig_SupTwgSgg_Token::STRING_TYPE)) || ($token = $stream->nextIf(Twig_SupTwgSgg_Token::NAME_TYPE)) || ($token = $stream->nextIf(Twig_SupTwgSgg_Token::NUMBER_TYPE))) {
        ...
        throw new Twig_SupTwgSgg_Error_Syntax(
          sprintf('A hash key must be a quoted string, a number, a name, or an expression enclosed in parentheses (unexpected token "%s" of value "%s".', Twig_SupTwgSgg_Token::typeToEnglish($current->getType()), $current->getValue()),
          $current->getLine(),
    ...
    switch ($name) {
      ...
      default:
        if (null !== ($alias = $this->parser->getImportedSymbol('function', $name))) {
          $arguments = new Twig_SupTwgSgg_Node_Expression_Array([], $line);
  ...
  public function parseSubscriptExpression($node)
  {
    ...
    $arguments = new Twig_SupTwgSgg_Node_Expression_Array([], $lineno);
    $type = Twig_SupTwgSgg_Template::ANY_CALL;
    if ($token->getValue() == '.') {
      $token = $stream->next();
      if ($token->getType() == Twig_SupTwgSgg_Token::NAME_TYPE || $token->getType() == Twig_SupTwgSgg_Token::NUMBER_TYPE || ($token->getType() == Twig_SupTwgSgg_Token::OPERATOR_TYPE && preg_match(Twig_SupTwgSgg_Lexer::REGEX_NAME, $token->getValue()))) {
      ...
      if ($slice) {
        ...
        $arguments = new Twig_SupTwgSgg_Node([$arg, $length]);
  ...
  {
    if (!($node instanceof Twig_SupTwgSgg_Node_Expression_Constant || $node instanceof Twig_SupTwgSgg_Node_Expression_Array || $node instanceof Twig_SupTwgSgg_Node_Expression_Unary_Neg || $node instanceof Twig_SupTwgSgg_Node_Expression_Unary_Pos)) {
      return false;
    }
